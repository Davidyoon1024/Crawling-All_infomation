#' Download all listed firm's price in United state Markets.
#'
#' This function will Download all listed firm's price in US Markets.
#' (NYSE, NASDAQ, AMEX Market - Recently, not support tickers of USA, So using data tool)
#'
...
#' @examples
#' \dontrun{
#'  US_price = get_USA_prices()
#'  }
#' @export

get_USA_prices = function() {
  library(xts)

  folder_name = "US_price"
  ticker = read.csv("./Projects/US_ticker.csv")
  ticker$Symbol = gsub(" ", "", ticker$Symbol)
